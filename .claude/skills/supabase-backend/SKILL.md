...
| Table                    | Key                   | Notes |
|--------------------------|-----------------------|-------|
| `trades`                 | `id uuid` (PK, server-generated) | One row per trade. `user_id` FK. `setup` (text, one of `rules.setup_options`), `market_cap` (text: `'Large Cap'\|'Mid Cap'\|'Small Cap'`, nullable). `segment` includes `'Index Future'` alongside Equity/F&O-Futures/F&O-Options/Commodity — its own capital bucket, not shared with F&O. |
...
| `capital`                | `user_id uuid` (PK)   | One row per user. Leverage caps + 6 position-sizing multiplier columns (`mult_large_cap`/`mult_mid_cap`/`mult_small_cap`, `mult_risk_on`/`mult_neutral`/`mult_risk_off`) + 4 capital-allocation columns (`alloc_equity_pct`/`alloc_fno_pct`/`alloc_comm_pct`/`alloc_indexfut_pct`, default 50/30/20/20) + `lev_indexfut` — a trade's risk % is measured against its own segment's allocated slice (`segmentCapital()` in `index.html`), not 100% of total capital for every segment. |
...
| `ema_state`               | `user_id uuid` (PK)   | One row per user holding a **jsonb map** of all ~500 NIFTY 500 symbols' `{ema50, ema200, lastClose, lastDate, recentCloses[6], recentVolumes[20]}` — not 500 rows, matches the `setup_options`-style jsonb-blob convention. `recentCloses`/`recentVolumes` are rolling windows feeding the 5-day-move and volume-confirmed-move stats. Also the sole data source (via a client-side `emaStateCache`) for the Sector Strength panel — no new table needed there. |
| `market_breadth_history` | `(user_id, snapshot_date)` (PK) | Date-keyed, always auto-updates (no user click possible) — see `nse-market-data` skill for why it's a separate table from `market_regime` rather than shared columns (a *different* reason than `market_regime`'s own auto-save: this one never had a button, `market_regime` had one removed by request). Also holds `count_up20_5d`/`count_up30_5d`/`count_up4pct_vol`/`count_down4pct_vol`. Backfilled to ~500 rows (2024-08-08 onward) as of this round — `fetchBreadthHistory()`'s query `.limit()` must stay comfortably above the row count or the extra history silently doesn't reach the charts (bit us once already this round). |
