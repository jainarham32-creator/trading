...
## Schema reference
Full DDL: `supabase/schema.sql` (fresh-install baseline) + `supabase/migration_00N_*.sql` (002 setup/regime/sizing, 003 EMA breadth, 004 segment allocation + 5-day/volume breadth extras, 005 Index Future bucket + Nifty 500 EMA20, 006 Nifty-500 new-highs/lows + Market Thrust advance/decline — what was actually run against the live project). Summary:
...
| `rules`                  | `user_id uuid` (PK)   | One row per user. `setup_options` (jsonb array of strings), `setup_multipliers` (jsonb, `{name: multiplier}`) added alongside the original free-text playbook columns. Two of those free-text columns (`leverage`, `setups`) are no longer shown in the UI as of this round (`ruleFields` in `index.html` trimmed from 8 to 6 — they duplicated things that now have real number/dropdown inputs alongside them in the same Playbook modal) but are left in place, unused, same "don't force a destructive migration" convention as `capital.lev_comm` below. |
...
| `market_regime`          | `id uuid` (PK) + `unique(user_id, snapshot_date)` | A date-keyed history. **Auto-saves** on every successful live fetch (tab-open + manual "Refresh from NSE") — this reverses an earlier deliberate no-auto-save decision, by explicit user request (see decisions log). `pcr` is auto-fetched (see `nse-market-data` skill); `notes` column is unused by the UI (removed) but left in place. `nifty500_close`/`nifty500_ema20` track the Nifty 500 index's own daily close and its incremental 20-EMA — one more factor in the Regime Score, seeded once via `scripts/backfill_nifty500_ema20.py` then rolled forward daily the same way `ema_state` is, just scoped to this table instead. `new_highs`/`new_lows` are no longer written (legacy/unused as of this round — see `market_breadth_history`'s `count_newhigh_n500`/`count_newlow_n500` for the replacement). Upsert with `onConflict:'user_id,snapshot_date'`. |
| `ema_state`               | `user_id uuid` (PK)   | One row per user holding a **jsonb map** of all ~500 NIFTY 500 symbols' `{ema50, ema200, lastClose, lastDate, recentCloses[6], recentVolumes[20], rsCloses[66], close252[252]}` — not 500 rows, matches the `setup_options`-style jsonb-blob convention. `recentCloses`/`recentVolumes` feed the 5-day-move and volume-confirmed-move stats; `rsCloses` (~3-month window) feeds the Sector Strength panel's RS Rating; `close252` (added this round, ~52-week window) feeds the Nifty-500-scoped new-high/new-low count. Also a data source (via a client-side `emaStateCache`) for the Sector Strength panel's RS Rating drill-down and its offline fallback — its *primary* live number moved to `api/regime.js`'s `allIndices`-derived `sectorAdvDecl` this round (see `nse-market-data` skill), so this table is no longer the sole source there. |
| `market_breadth_history` | `(user_id, snapshot_date)` (PK) | Date-keyed, always auto-updates (no user click possible) — see `nse-market-data` skill for why it's a separate table from `market_regime` rather than shared columns (a *different* reason than `market_regime`'s own auto-save: this one never had a button, `market_regime` had one removed by request). Also holds `count_up20_5d`/`count_up30_5d`/`count_up4pct_vol`/`count_down4pct_vol` (the latter two's UI chart was removed by request in an earlier round, columns/computation untouched) and, added this round, `count_newhigh_n500`/`count_newlow_n500` (Nifty-500-scoped 52-week new highs/lows, backfilled historically) and `count_advances`/`count_declines` (whole-market Market Thrust, **not** backfilled — a today-only snapshot stat, only ever written going forward by `updateBreadthIfNeeded()`). Backfilled to ~4 years (1051 rows, 2022-06-29 to present) — `fetchBreadthHistory()`'s query `.limit()` (1500 as of this round) must stay comfortably above the row count or the extra history silently doesn't reach the charts (bit us once already); separately, Supabase's own project-level `db-max-rows` (default 1000) caps every response regardless of the client `.limit()`, so the charts currently show ~1000 of the 1051 rows — see `nse-market-data` skill. |
| `watchlist`              | `user_id uuid` (PK)   | One row per user, `symbols` (jsonb array of strings) — same one-row-per-user/jsonb-array shape as `rules.setup_options`. Added this round for the Stocks tab's "My Watchlist" panel; feeds `api/news.js`'s optional `?watchlist=` query param (`fetchNews()` in `index.html` builds it from this array) so that panel's news stays scoped to what the user actually added. Migration 007. |
...
- The standalone Exposure tab was merged into Journal this round (`view-exposure` removed, its content redistributed into `view-journal`'s 3-column layout) and its Capital & Leverage form moved into the Playbook modal. **Purely a UI reorganization — no schema change, no new table, no column added or dropped.** `capital`/`rules` are read/written exactly as before; only which DOM elements the same input IDs live inside changed.
